...
// Transportador de correo optimizado para Render (evita bloqueo de puertos y timeouts)
const transporter = nodemailer.createTransport({
  ...
  connectionTimeout: 10000, // Máximo 10 segundos para conectar
...
app.post('/api/donate', async (req, res) => {
  ...
  try {
    // 1. Guardar primero en la nube (Base de datos)
    const newDonation = {
      id: Date.now(),
      name,
      email,
      ...
      bank: bank || 'N/A',
      status: 'pendiente', // Estado inicial para tu control
    ...
    // 2. Intentar enviar el correo por separado (Si falla, NO cancela el registro)
    try {
      const mailOptions = {
      ...
      // Si el correo falla en Render, se imprime el error aquí sin tumbar la aplicación
